...
function Ajax(recvType, waitId) {
    ...
    aj.loading = 'Please wait...';//note public
    ...
    //note loading when null, use the default, an empty string will not be displayed.
    aj.setLoading = function (loading) {
        if (typeof loading !== 'undefined' && loading !== null) aj.loading = loading;
    };

    // note defaults to XML mode
        ...
        if (aj.XMLHttpRequest.readyState == 4 && aj.XMLHttpRequest.status == 200) {
            ...
            if (aj.recvType == 'HTML') {
                ...
                if (!aj.XMLHttpRequest.responseXML || !aj.XMLHttpRequest.responseXML.lastChild || aj.XMLHttpRequest.responseXML.lastChild.localName == 'parsererror') {
                    aj.resultHandle('<a href="' + aj.targetUrl + '" target="_blank" style="color:red">Internal error, can not display this content</a>', aj);
                } else {
...
// note wrapper function parameters
...
function ajaxupdateevent(o) {
    if (typeof o == 'object' && o.getAttribute) {
        if (o.getAttribute('ajaxtarget')) {
            if (!o.id) o.id = Math.random();//note The tag uniquely identifies
            var ajaxevent = o.getAttribute('ajaxevent') ? o.getAttribute('ajaxevent') : 'click';//note The default is a click event
            var ajaxurl = o.getAttribute('ajaxurl') ? o.getAttribute('ajaxurl') : o.href;//note The default is a click event
...
/*
 * Url: url demand requests
   * Id: id display
   * Waitid: wait for the id, id displayed by default, if waitid an empty string, no show loading ..., if it is null, then the display area in showid
   * Linkid: ajax request which link is the trigger, attributes of the object (such as ajaxdisplay) holds the data of some ajax request process needs.
 */
...
function ajaxpost(formid, showid, waitid, showidclass, submitbtn, recall) {
        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    s = 'Internal error, can not display this content';
                }
            }
        }
    ...
    //note Avoid duplication create iframe, to determine whether the object exists IFRAME
    if (!ajaxframe) {
...
// note to get a fixed-length hash value, depending on stringxor ()
...
// note two strings XOR, the result is a combination of English characters
...
function bbcode2html(str) {
    ...
    str = str.replace(/\[@([\w-]{6,20}),?(\d+)?\]/ig, function ($1, $2, $3) {
        return '<iframe src="http://follow.v.t.qq.com/index.php?c=follow&a=quick&name=' + $2 + '&style=3&t=' + new Date().getTime() + '&f=1" width="' + (!$3 || $3 > 300 ? 168 : $3) + '" height="20" frameborder="0" scrolling="no" allowtransparency="true" style="margin:0 auto;"></iframe>';
    });
...
function getapp(param) {
    if (siteurl) {
        ...
        alert('Site Error, unable to install the application');
    }
}
...
function shareTo(type) {
    var shareURL = {
        ...
        'qzone': '"http://sns.qzone.qq.com/cgi-bin/qzshare/cgi_qzshare_onekey?site=' + encodeURI('Discuz! Application Center') + '&pics="+(_pic?_pic.join("|"):"")+"&"',
        'pengyou': '"http://sns.qzone.qq.com/cgi-bin/qzshare/cgi_qzshare_onekey?site=' + encodeURI('Discuz! Application Center') + '&to=pengyou&pics="+(_pic?_pic.join("|"):"")+"&"',
    ...
    _t = '%23' + encodeURI('Discuz Application Center') + '%23 ' + encodeURI(_t);
 ...
 * Select
 * Form form objects
 * Prefix name prefix, default empty
 * Checkall checkall object's own name, the default chkall
 */
...
function check_identifier(id, resultid) {
    ...
    x.get('develop.php?mod=plugin&action=edit&operation=check_identifier&id=' + $(id).value, function (s, x) {
            if (s == '1') {
                msg = '<font color="red">Plug-ID already exist, it will affect the public, in the proposed change</font>';
            } else if (s == '2') {
                msg = 'Plugin prefix is occupied, it does not affect the release, but the proposed change';
            } else if (s == '0') {
                msg = '<font color="green">Plugin prefix is not currently occupied</font>';
            } else {
                msg = 'Test failed, please try again later.';
            }
