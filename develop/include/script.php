...
if (!submitcheck('pluginsubmit'))
{
	...
	if ($type && $filename && (in_array($type, $basetype) && $plugin['modules'][$type] || $plugin['modules'][$type][$filename]))
	{
		...
		else
		{
			...
			if ($type == 'cron')
			{
				...
				$week = array('0' => 'On Sunday', '1' => 'Monday', '2' => 'Tuesday', '3' => 'Wednesday', '4' => 'Thursday', '5' => 'Friday', '6' => 'On Saturday');
			...
			{
				$allowgroup = array('0' => 'Ordinary users', '1' => 'Administrator', '2' => 'Super Moderator', '3' => 'Withheld');
			}
		}
	}
	...
	$navtypes = array('g1' => array('1' => array('h' => '1100100', 'e' => 'inc', 'value' => '1', 'desc' => $devlang['plugins_edit_modules_type_1']), '5' => array('h' => '1111', 'e' => 'inc', 'value' => '5', 'desc' => $devlang['plugins_edit_modules_type_5']), '27' => array('h' => '1100100', 'e' => 'inc', 'value' => '27', 'desc' => $devlang['plugins_edit_modules_type_27']), '23' => array('h' => '1100100', 'e' => 'inc', 'value' => '23', 'desc' => $devlang['plugins_edit_modules_type_23']), '25' => array('h' => '1100110', 'e' => 'inc', 'value' => '25', 'desc' => $devlang['plugins_edit_modules_type_25']), '24' => array('h' => '1100111', 'e' => 'inc', 'value' => '24', 'desc' => $devlang['plugins_edit_modules_type_24'])), 'g3' => array('7' => array('h' => '1111', 'e' => 'inc', 'value' => '7', 'desc' => $devlang['plugins_edit_modules_type_7']), '17' => array('h' => '1111', 'e' => 'inc', 'value' => '17', 'desc' => $devlang['plugins_edit_modules_type_17']), '19' => array('h' => '1111', 'e' => 'inc', 'value' => '19', 'desc' => $devlang['plugins_edit_modules_type_19']), '14' => array('h' => '1001', 'e' => 'inc', 'value' => '14', 'desc' => $devlang['plugins_edit_modules_type_14']), '26' => array('h' => '1111', 'e' => 'inc', 'value' => '26', 'desc' => $devlang['plugins_edit_modules_type_26']), '21' => array('h' => '1111', 'e' => 'inc', 'value' => '21', 'desc' => $devlang['plugins_edit_modules_type_21']), '15' => array('h' => '1001', 'e' => 'inc', 'value' => '15', 'desc' => $devlang['plugins_edit_modules_type_15']), '16' => array('h' => '1001', 'e' => 'inc', 'value' => '16', 'desc' => $devlang['plugins_edit_modules_type_16']), '3' => array('h' => '1001', 'e' => 'inc', 'value' => '3', 'desc' => $devlang['plugins_edit_modules_type_3'])));
}
else
{
	...
	if (submitcheck('addhook'))
	{
					...
					foreach ($value as $fkey => $hval)
					{
						if ($sorts[$fkey])
						{
							foreach ($hval as $hook)
							{
								...
								//Select the primary method to determine whether there is, if not abandoned output hook
		...
		devmessage('Embedding point has been added, continue to the next step.', "develop.php?mod=plugins&action=$action&operation=$operation&pluginid=$pluginid&filename=$filename&type=$type", 'succeed');
	}
	elseif (submitcheck('editcron'))
	{
		...
		//Write plug-in information
		C::t('common_plugin')->update($pluginid, array('modules' => serialize($plugin['modules'])));
		devmessage('Scheduled tasks setup is complete', "develop.php?mod=plugins&action=$action&operation=$operation&pluginid=$pluginid", 'succeed');
	}
	elseif (submitcheck('editadv'))
	{
		...
		//Write plug-in information
		C::t('common_plugin')->update($pluginid, array('modules' => serialize($plugin['modules'])));
		devmessage('Script editing is complete', "develop.php?mod=plugins&action=$action&operation=$operation&pluginid=$pluginid", 'succeed');
	}
	elseif (submitcheck('editnav'))
	{
		...
		$plugin['modules'][$type][$filename] = array('name' => $_GET['name'], 'menu' => trim($_GET['menu']), 'url' => trim($_GET['url']), 'type' => intval($_GET['newtype']), 'adminid' => $_GET['adminid'] >= 0 && $_GET['adminid'] <= 3 ? intval($_GET['adminid']) : 1, 'displayorder' => intval($_GET['order']), 'navtitle' => $_GET['navtitle'], 'navicon' => $_GET['navicon'], 'navsubname' => $_GET['navsubname'], 'navsuburl' => $_GET['navsuburl'], 'description' => trim($_GET['menu']));
		//Write plug-in information
		C::t('common_plugin')->update($pluginid, array('modules' => serialize($plugin['modules'])));
		devmessage('Script editing is complete', "develop.php?mod=plugins&action=$action&operation=$operation&pluginid=$pluginid", 'succeed');
	}
	elseif (submitcheck('editsystem'))
	{
		if (in_array($filename, array('install', 'uninstall', 'upgrade')))
		{
			...
			//While increasing the delete SQL table when you add a table
	...
	else
	{
		...
		if ($action == 'edit')
		{
			devmessage('Script successfully added', "develop.php?mod=plugins&action=$action&operation=script&pluginid=$pluginid", 'succeed');
		}
		else
		{
			dheader("location:develop.php?mod=plugins&action=$action&operation=setting&pluginid=$pluginid");
			//devmessage ('script adds success', "?develop.php mod=plugins&action=$action&operation=setting&pluginid=$pluginid", 'succeed');
		}
	}
}
?>